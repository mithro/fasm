...
# Where each Xilinx design comes from and under which terms (T7.4
# review). VTR's LICENSE.md (25e723a24) puts only VTR's own code under
# MIT: "The benchmark circuits are all open source but each have their own
# individual terms and conditions which are listed in the source code of
# each benchmark." The symbiflow task's eblif netlists carry no licence
# text; their Yosys `src` attributes name the sources, whose projects'
# licences are given (read from each project's licence file or source
# header).
SYMBIFLOW_NOTE = (
    'The eblif netlist (symbiflow-arch-defs `fb1b251a` benchmark tarball) '
    'is Yosys output and carries no licence text; its `src` attributes name '
    'the sources above.')
SOURCES = {
    'counter_basys3': (
        'symbiflow-arch-defs test `xc/xc7/tests/counter/counter_basys3.v`',
        'ISC (symbiflow-arch-defs `COPYING`)'),
    'picosoc_basys3_full_50': (
        'symbiflow-arch-defs `tests/9-soc/picosoc/` (`basys3-full_demo_50.v`,'
        ' `firmware_noflash_50.v`) around PicoSoC / PicoRV32 (`picorv32.v`, '
        '`picosoc_noflash.v`, `simpleuart.v`, from YosysHQ/picorv32)',
        'ISC: PicoRV32 and PicoSoC "Copyright (C) 2015/2017 Claire Xenia '
        'Wolf" with the ISC permission notice in their source headers; '
        'symbiflow-arch-defs `COPYING` (ISC)'),
    'murax_basys3_full_50': (
        'symbiflow-arch-defs `tests/9-soc/murax/` (`basys3-full_toplevel_50.v`'
        ') around `Murax.v`, the Murax SoC generated by SpinalHDL/VexRiscv',
        'MIT (VexRiscv `LICENSE`, "Copyright (c) 2016 Spinal HDL '
        'contributors"); symbiflow-arch-defs `COPYING` (ISC) for the '
        'wrapper'),
    'ibex_arty': (
        'symbiflow-arch-defs `xc/xc7/tests/soc/ibex/ibex_sv2v.v`: lowRISC '
        'Ibex converted to Verilog with sv2v',
        'Apache-2.0 (lowRISC/ibex `LICENSE`)'),
    'minilitex_arty': (
        'symbiflow-arch-defs `xc/xc7/tests/soc/litex/mini/`: a LiteX '
        'generated SoC (`gateware/top.v`) with the VexRiscv CPU of '
        'litex-hub/pythondata-cpu-vexriscv (`VexRiscv.v`)',
        'BSD-2-Clause (LiteX `LICENSE`) and MIT (pythondata-cpu-vexriscv '
        '`LICENSE`, VexRiscv)'),
    'linux_arty': (
        'symbiflow-arch-defs `xc/xc7/tests/soc/litex/linux/`: a LiteX '
        'generated SoC (`gateware/top.v`) with `VexRiscv_Linux.v` of '
        'litex-hub/pythondata-cpu-vexriscv',
        'BSD-2-Clause (LiteX `LICENSE`) and MIT (pythondata-cpu-vexriscv '
        '`LICENSE`, VexRiscv)'),
    'sha': (
        'VTR `vtr_flow/benchmarks/verilog/sha.v`: the OpenCores `sha_core` '
        '(SHA-160) by marsgod',
        '"Copyright (C) 2002-2004 marsgod": "This source file may be used '
        'and distributed without restriction provided that this copyright '
        'statement is not removed from the file and that any derivative '
        'work contains the original copyright notice and the associated '
        'disclaimer" (OpenCores notice in the source header, with an AS IS '
        'disclaimer)'),
    'diffeq2': (
        'VTR `vtr_flow/benchmarks/verilog/diffeq2.v`: a differential '
        'equation solver by P. Sridhar, University of Cincinnati (1991, from '
        'the High-Level Synthesis Workshop repository; HardwareC original by '
        'Rajesh Gupta, Stanford)',
        'no licence in the source header, only a disclaimer ("This comes '
        'with absolutely no guarantees of any kind"); distributed with VTR '
        'as a benchmark circuit'),
    'spree': (
        'VTR `vtr_flow/benchmarks/verilog/spree.v`: a MIPS I soft '
        'processor (SPREE)',
        'no author or licence notice in the source; distributed with VTR as '
        'a benchmark circuit'),
}
for _c in ('picosoc_basys3_full', 'murax_basys3_full'):
    _source, _licence = SOURCES[_c + '_50']
    SOURCES[_c + '_100'] = (_source.replace('_50', '_100'), _licence)
for _c in ('minilitex_ddr_arty', 'minilitex_ddr_eth_arty'):
    SOURCES[_c] = (SOURCES['minilitex_arty'][0].replace(
        'litex/mini/', 'litex/mini_%s/' % _c[len('minilitex_'):-len('_arty')]),
        SOURCES['minilitex_arty'][1])
for _c in ('and_latch', 'single_ff', 'single_wire',
           'multiclock_output_and_latch', 'multiclock_reader_writer',
           'multiclock_separate_and_latch'):
    SOURCES[_c] = (
        'VTR `vtr_flow/benchmarks/verilog/%s.v`, one of VTR\'s small test '
        'designs' % _c,
        'no licence or author notice in the source (VTR\'s `LICENSE.md` '
        'leaves benchmark circuits to the terms in their source, and this '
        'one states none); distributed with VTR')
HARD_BLOCK_NOTE = (
    'The VTR hard block models (`vtr_hard_blocks.v`) are VTR\'s own '
    '`vtr_flow/primitives.v` (MIT, VTR `LICENSE.md`), the RAMs rewritten '
    'by `tools/e2e/vtr/hard-block-models.py` (Apache-2.0, this '
    'repository).')


def source_section(circuit, verilog, hard_blocks):
    """The README's "Source and licence" section of a Xilinx design."""
    source, licence = SOURCES[circuit]
    lines = ['## Source and licence', '',
             '* Design: ' + source + '.', '* Licence: ' + licence + '.']
    if not verilog:
        lines.append('* ' + SYMBIFLOW_NOTE)
    if hard_blocks not in ('', 'none'):
        lines.append('* ' + HARD_BLOCK_NOTE)
    lines.append('* The FASM, frames and bitstream are tool output '
                 '(VPR/genfasm, xcfasm) of this design.')
    return '\n'.join(lines) + '\n\n'
...
def install_xilinx(src, info):
    ...
    common = COMMON.replace('## Tools\n',
                            source_section(circuit, 'top' in info,
                                           hard_blocks).replace(
                                               '{', '{{').replace(
                                                   '}', '}}') + '## Tools\n',
                            1)
