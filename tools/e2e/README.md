...
## RapidWright cross-checks (T7.5)

RapidWright (Xilinx/AMD's open source Java framework, `v2026.1.0-beta`)
has no FASM writer and no bit database, but its closed source
`com.xilinx.rapidwright.bitstream` package knows every part's
configuration array and reads and writes `.bit` files, and its FPGA
interchange writer feeds python-fpga-interchange's xc7 FASM generator.
What it can and cannot do here (no Vivado): `docs/rewrite/
DESIGN-rapidwright.md`; results matrix: `docs/rewrite/DESIGN-xilinx-db.md`
§8.15; RapidWright-vs-prjxray reader differences: `docs/rewrite/
COMPAT.md`, "Bitstream readers compared with RapidWright".
...
standalone jar (115 MB, sha256 checked), the RapidWright device files of
the ten devices behind our databases and test bitstreams (30 MB; md5 as
RapidWright checks it, and sha256), the compiled drivers
(`tools/e2e/rapidwright/RwCheck.java`, `RwDesign.java`), and with
`--with-interchange` a venv with python-fpga-interchange (pycapnp 1.3),
the interchange schema, `java.capnp`, python-fpga-interchange's Series7
device patches and nine DCPs of RapidWright's test data. Java 11+ (21
used), `-Xmx4g`. RapidWright caches derived device data next to the
device files (`*_db_cache.dat`, a few MB each) on first use.

### Running

```sh
cargo build --release
tools/e2e/run-rapidwright-checks.sh --quick          # 3 parts, 11 cases: ~1 min
tools/e2e/run-rapidwright-checks.sh layout bits      # everything but fasm: ~15 min
tools/e2e/run-rapidwright-checks.sh fasm             # needs --with-interchange and
                                                     # tests/oracle/setup-xilinx.sh: ~3 min
```

`$FASM_DB_CACHE` (databases), `$FASM_ORACLE_SRC` (the reference sources
of `tests/oracle/setup-xilinx.sh`, for the Vivado test bitstreams of
prjxray and prjuray-tools), `$FASM_ORACLE_DIR` (the reference
`fasm2frames`) and `$RAPIDWRIGHT_FLOW_OUT` (colon separated directories
with flow built `*.bit` files, default `tools/e2e/build/out`) select the
inputs; `--parts`, `--cases` narrow the run. The report is
`tools/e2e/build/rapidwright-work/report.json`; every case is
`identical`, `explained` (a RapidWright behaviour verified exactly, see
COMPAT.md), `not possible` (the FASM route cannot handle the design, with
the error) or `different` (exit status 1). `rwcheck.py layout
--write-golden` rewrites the committed layouts and `rwcheck.py fasm
--install` the committed FASM designs
(`tests/corpus/xilinx/*/designs/rapidwright/`).

### Tests

`tests/e2e/test_rapidwright.py`: the committed layouts are consistent;
with the Rust tools and databases, every part's Rust walk, columns,
IDCODE and FDRI size equal the committed RapidWright layout, the stored
FASM designs assemble to their reference frames, and the prjxray reader's
lost last frame of a per frame CRC bitstream is kept; with RapidWright,
`--quick` has no difference, and with the interchange route the FASM
designs are regenerated byte for byte.
