...
| `xilinx/{artix7,zynq7}/designs/rapidwright/` | 712K | FASM of designs placed and routed by RapidWright (`tools/e2e/rapidwright/RwDesign.java`) and of two Vivado DCPs of RapidWright's test data (Xilinx/RapidWrightDCP), converted without Vivado through the FPGA interchange and python-fpga-interchange's xc7 FASM generator, with their reference sparse frames and a README each (T7.5) | Apache-2.0 (this repository's driver; RapidWrightDCP) |
| `xilinx/<family>/rapidwright/`, `prjuray/zynqusp/rapidwright/` | 180K | RapidWright's configuration array layout of every device we have a database for (frame address walk hash, columns, frame counts, IDCODE; `tools/e2e/rapidwright/rwcheck.py layout --write-golden`, T7.5; see `xilinx/artix7/rapidwright/README.md`) | generated device data |
...
**Total: ~28M** (`du -sh tests/corpus`; almost all of it the toolchain
