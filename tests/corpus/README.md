...
| `xilinx/artix7/` (without `designs/`) | 280K | prjxray-db derived hand written fixture (`smoke_x1y0.fasm`, predates T1.5, see its own `README.md`), `synthetic/` (hand written `fasm2frames` cases and the error corpus below, T5.4), `generated/` (golden reference results for `tools/gen-xilinx-corpus.py`'s corpus of one part, T5.9) and its own `rapidwright/` (see the row below) | Apache-2.0 |
| `xilinx/<family>/designs/f4pga-examples/` | 8.3M | FASM (openXC7 `top.fasm` of T7.1, f4pga/VPR `vpr.fasm` of T7.3) of the chipsalliance/f4pga-examples designs, with frames and a README each (artix7 and zynq7) | Apache-2.0 |
...
| `xilinx/{artix7,zynq7}/designs/rapidwright/` | 660K | FASM of designs placed and routed by RapidWright (`tools/e2e/rapidwright/RwDesign.java`), converted without Vivado through the FPGA interchange and python-fpga-interchange's xc7 FASM generator, with their reference sparse frames and a README each (T7.5). Nothing derived from RapidWright's test DCPs (Xilinx/RapidWrightDCP, no licence file) is stored | Apache-2.0 (generated by this repository's driver, no third party design sources) |
| `xilinx/<family>/rapidwright/` (`artix7`, `kintex7`, `kintexu`, `spartan7`, `zynq7`), `prjuray/zynqusp/rapidwright/` | 204K | RapidWright's configuration array layout of every device we have a database for (frame address walk hash, columns, frame counts, IDCODE; `tools/e2e/rapidwright/rwcheck.py layout --write-golden`, T7.5; see `xilinx/artix7/rapidwright/README.md`) | generated device data |
...
| `prjuray/` | 44K | golden reference results (`uray-fasm2frames`) for the generated corpus of one UltraScale+ part (`zynqusp/generated/`, T6.3), plus its own `zynqusp/rapidwright/` (see the row above) | Apache-2.0 |
