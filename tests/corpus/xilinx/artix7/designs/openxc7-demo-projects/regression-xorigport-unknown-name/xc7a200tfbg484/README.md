# openxc7-demo-projects/regression-xorigport-unknown-name/xc7a200tfbg484 -- nextpnr-xilinx (openXC7) FASM (T7.6)

`top.fasm` is the FASM nextpnr-xilinx wrote for openXC7 demo-projects `regression/xorigport-unknown-name` ([source](https://github.com/openXC7/demo-projects/tree/c5246c583a7db3a73543af72e00193f2fa990d34/regression/xorigport-unknown-name), commit `c5246c583a7db3a73543af72e00193f2fa990d34`), built by `tools/e2e/run-nextpnr-examples.sh openxc7-demo-projects/regression-xorigport-unknown-name/xc7a200tfbg484` following the example's own regression runner (`regression/run.sh`) (see `tools/e2e/README.md`, "nextpnr-xilinx examples corpus (T7.6)"). `top.frm.xz` is the flow's frames: the snap's `fasm2frames`, dense, with the snap's prjxray-db.
...
* FASM: 63 lines, 2380 bytes
* Flow times on this machine (4 cores, s): buf_fix 0.66, fasm2frames 1.87, pnr 4.24, synth 2.67, xc7frames2bit 0.26

* Note: $buf cells removed with techmap (yosys workaround); regression run.sh criteria: check.sh FAILS

* The case's `check.sh` against this nextpnr-xilinx (`check.log`):

```
renamed I0->A1 in $abc$20560$lut$aiger$o2 (LUT2)
FAIL: nextpnr accepted an unknown X_ORIG_PORT name and encoded it as I0
```
...
```
sha256  top.fasm  7dfbea4226570a1bd1efc0ca359f06c288095bf7a096606133227cd7b6866930
sha256  top.frm   ce54bc9f71ac531dd91e8c9af2e29a93997299bd479a3634b95af8380b64f97a  (22698060 bytes)
sha256  top.bit   19997f42ffcc447c41ee30c1eeddf4f09fdeebedbc2ae930203d3635b1e8ac7b  (9730754 bytes; header with the build time)
```
