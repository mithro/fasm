...
* Version: static `0.1.0.dev0` in `pyproject.toml` for now (TODO T3.2: the
  `git describe` scheme of `update_version.py`). `fasm/version.py` is in
  `.gitignore`, so maturin does not package it; `fasm.__version__` then
  comes from the package metadata.
