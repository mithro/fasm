...
| `dir(fasm)` | `TypeError: 'str' object is not callable` (`fasm/__init__.py` assigns its package directory to the module-level name `__dir__`, which Python's `dir()` protocol then tries to call) | works; the package directory is `fasm.PACKAGE_DIR` instead (deliberate fix, nothing relies on the crash). `fasm.__dir__` itself now resolves to the module type's ordinary built-in `__dir__` method (no longer a plain string) |
