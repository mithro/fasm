...
//!
//! Generates a synthetic FASM file of feature lines (default 100 MB, set
//! `FASM_PARSER_BENCH_MB` to change it) shaped like real Xilinx 7 series
//! output: routing pips, single bit features and multi bit LUT `INIT`
//! values over a grid of tiles. Alternatively set `FASM_PARSER_BENCH_FILE`
//! to parse an existing file.
//!
...
/// Builds about `target_bytes` of FASM feature lines.
...
fn run(name: &str, data: &[u8]) {
    ...
    println!(
        "{name:>5}: {mb:8.1} MB, {lines:9} lines, {:7.3} s, {:7.1} MB/s, {:6.1} ns/line",
        elapsed,
        mb / elapsed,
...
fn main() {
    let data = if let Ok(path) = std::env::var("FASM_PARSER_BENCH_FILE") {
        ...
        let mb: usize = std::env::var("FASM_PARSER_BENCH_MB")
            ...
            .unwrap_or(100);
        generate(mb * 1_000_000).into_bytes()
    };
    run("cold", &data);
    run("warm", &data);
    run("warm", &data);
